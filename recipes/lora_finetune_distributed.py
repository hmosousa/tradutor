...
class LoRAFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_model(
        ...
        model = FSDP(
            module=model,
            auto_wrap_policy=utils.lora_fsdp_wrap_policy(modules_to_wrap={modules.TransformerDecoderLayer}),
    ...
    def _setup_optimizer(self, cfg_optimizer: DictConfig, opt_state_dict: Optional[Dict[str, Any]] = None) -> Optimizer:
        ...
        if opt_state_dict:
            ...
            opt_state_dict = utils.transform_opt_state_dict(opt_state_dict, self._model, optimizer)
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    init_process_group(backend="gloo" if cfg.device == "cpu" else "nccl", timeout=timedelta(seconds=7200000))
